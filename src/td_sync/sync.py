    ...
    if 'tabsdata/api/tabsdata_server.py' not in str(call_stack):
        ...
        while collection_validated_flag == False:
            ...
            if collection_name is None:
                ...
                f"or define a new collection to create and use:\n\n" +
                "\n" + options_string +
                "\n\n> ")
                ...
                if pattern.match(collection_name) is None:
                    if collection_name.isdigit():
                        if int(collection_name) in options_dict:
                            ...
                            collection_name = input('Index not found. Please enter a valid collection name or valid index: ')  
            #Handles if collection name provided is not valid
            elif pattern.match(collection_name) is None:
                 collection_name = None 
                 print('Collection Name provided is not valid. Please enter a valid collection name or valid existing collection name or index:')   
        ...
        if file_name in functions_in_file:
            ...
            function_name = input("No functions with name matching the file name found. Please enter the function name to proceed: ")
        ...
        if check_function_exists:
            server.update_function(collection_name, function_name, function_path, description='updated through auto-updater')
            print(f'Updated Function with the following parameters\n\nCollection: {collection_name}\nFunction Name: {function_name}')
        else:
